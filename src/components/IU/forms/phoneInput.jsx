...
export default function PhoneInput({
  ...
  return (
    <div className="flex w-full items-center rounded-md border border-gray-300 bg-white focus-within:ring-2 focus-within:ring-cyan-500 dark:border-gray-600 dark:bg-gray-800">
      <div ref={dropdownRef} className="relative">
        <button
          type="button"
          onClick={() => !disabled && setIsOpen((prev) => !prev)}
          className="flex w-16 items-center justify-center gap-1 rounded-l-md border-r border-gray-300 bg-gray-50 px-1.5 py-2 focus:outline-none disabled:opacity-50 dark:border-gray-600 dark:bg-gray-700"
          disabled={disabled}
        ...
        {isOpen && (
          <div className="absolute left-0 top-full z-20 mt-1 grid w-40 grid-cols-5 gap-1 rounded-md border border-gray-300 bg-white p-2 shadow-lg dark:border-gray-600 dark:bg-gray-800">
            {COUNTRY_OPTIONS.map((country) => (
              <button
                key={country.value}
                type="button"
                onClick={() => selectCountry(country.value)}
                className={`rounded p-1 text-lg leading-none transition hover:bg-cyan-100 dark:hover:bg-gray-700 ${
                  selectedCountry.value === country.value ? 'bg-cyan-50 dark:bg-gray-700' : ''
                }`}
                title={country.label}
      ...
      <span className="px-2 text-sm font-medium text-gray-700 dark:text-gray-200">
